...
const useAdminVideosStore = create((set, get) => ({
  ...
  async fetchAll(force = false) {
    ...
    try {
      const { data } = await axios.get(`${API_URL}/video`, { withCredentials: true });
  ...
  async remove(id) {
    await axios.delete(`${API_URL}/video/${id}`, { withCredentials: true });
  ...
  async submitCreate() {
    ...
    const { data } = await axios.post(`${API_URL}/video`, fd, {
  ...
  async submitUpdate(id) {
    ...
    const { data } = await axios.put(`${API_URL}/video/${id}`, fd, {
      withCredentials: true,
      headers: { 'Content-Type': 'multipart/form-data' },
    });
    const saved = data?.item || data;
    set({
      items: get().items.map((x) => ((x.id ?? x._id) === (saved.id ?? saved._id) ? saved : x)),
    });
    return saved;
  },
}));
